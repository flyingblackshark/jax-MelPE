...
class MelSpectrogram(torch.nn.Module):
    ...
    def forward(self, audio, keyshift=0, speed=1, center=True):
        ...
        fft = torch.stft(
            ...
            return_complex=True)
        #audio = audio.numpy()
        #audio = jax.device_put(audio, jax.devices('cpu')[0])
        #audio = jdp.from_dlpack(tdp.to_dlpack(audio.contiguous()))
        #audio = audio.numpy()
        #fft = scipy.signal.stft(audio,nfft=n_fft_new,nperseg=win_length_new,noverlap=n_fft_new-hop_length_new,padded=False,boundary=None)
        #fft = jax.scipy.signal.stft(audio,nfft=n_fft_new,nperseg=win_length_new,noverlap=n_fft_new-hop_length_new,padded=False,boundary=None)
        #magnitude = np.abs(fft[2])
        #magnitude = torch.tensor(np.asarray(magnitude))
        # magnitude = tdp.from_dlpack(jdp.to_dlpack(magnitude))
        ...
        #log_mel_spec = torch.log(mel_output)
